...
久坐检测器载板 PCB (KiCad 10). **ESP32-WROOM-32** + **GY-87** + PM11。载板 J2 为 GY-87 8Pin 排母（见 `hardware/HARDWARE.md` J2 表）；**v0.4 PCB 待按 GY-87 改 J2 走线后打样 v0.5**。

## Design authority

`hardware/HARDWARE.md` 是网络、引脚映射、BOM、PCB 约束的唯一权威来源。生成原理图或 PCB 前必须先读该文件。**但以下简表优先于 HARDWARE.md**（以下为更新后的权威值，HARDWARE.md 可能滞后）。

### J1 ESP32 引脚映射（丝印名称，上→下）
...
> D25/D26 等未用 GPIO 在载板上不连接。

### J2 GY-87 引脚映射（Pin1=VCC_IN）

| J2 Pin | GY-87 丝印 | 网络 | 接 ESP32 |
|--------|-----------|------|----------|
| 1 | **VCC_IN** | 3V3 | Pin30 (3V3) |
| 2 | 3.3V | — | NC |
| 3 | GND | GND | Pin14/29 (GND) |
| 4 | SCL | I2C_SCL | Pin23 (D5) |
| 5 | SDA | I2C_SDA | Pin26 (D4) |
| 6 | FSYNC | — | NC |
| 7 | INTA | MPU_INT | Pin27 (D2) |
| 8 | DRDY | — | NC |
...
| 元件 | 引脚 | 网络 | 接 J1 |
|------|------|------|-------|
| J4 | Pin1(VCC) | 3V3 | Pin30 |
| J4 | Pin2(GND) | GND | Pin14/29 |
| J4 | Pin3(SCL) | I2C_SCL | Pin23(D5) |
| J4 | Pin4(SDA) | I2C_SDA | Pin26(D4) |

I2C 共享: MPU6050=0x68, SSD1306 OLED=0x3C, 不冲突。载板无按键/滑动开关。
...
| J2 GY-87 1×8 排母 | `Connector_PinSocket_2.54mm:PinSocket_1x08_P2.54mm_Vertical` | Pin1=VCC_IN … Pin8=DRDY，见上表 |
...
- [x] 原理图 v0.6：J1/J2/J3/J4 仅 MCU+IMU+OLED+电源（无载板按键/开关）
- [x] PCB 丝印 v0.5.2：J1 左右 Dxx 标签
- [x] PCB 网络分配：J1/J2/J3/J4
- [x] 固件 GY-87 杜邦线联调通过 (→ `../04-MCU-GY87-Debug`)
- [ ] PCB 走线：全板待布
- [ ] DRC 通过
- [ ] 固件：OLED 显示 (→ `../03-ESP32-OLED-Button-Demo`，无载板按键)
- [ ] 固件：GY-87 传感器调试 (→ `../04-MCU-GY87-Debug`)
