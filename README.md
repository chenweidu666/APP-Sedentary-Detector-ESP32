...
基于 **ESP32-WROOM-32** + **GY-87**（10DOF，固件用 MPU6050）+ OLED 的椅子久坐检测器。安装在椅子坐垫下方，通过检测坐垫倾斜角度变化判断是否有人坐，超时提醒起身活动。PM11 锂电包供电，USB 充电，**PM11 自带开关**控制电源。
...
| 模块 | 状态 |
|------|------|
| 硬件方案 | ✅ 已完成 (ESP32-WROOM-32 + **GY-87** + PM11) |
| 载板 PCB 设计 | 🔧 v0.4 待改版 v0.5（J2 改 GY-87 引脚顺序） |
| Wokwi 仿真 | ✅ 已完成 (ESP32 + MPU6050, 检测算法验证) |
| 固件开发 | 🔧 联调中 (GY-87 / MPU6050 已验证，竖放 ≈90°) |
| 3D 底座 | ⏳ 待设计 |
| 采购 | ⏳ 待下单 |
...
| IMU 模块 | **GY-87**（10DOF，固件用 MPU6050） | ~¥30 | 含 BMP180(`0x77`)，I2C 见 HARDWARE.md |
| 电池模块 | PM11锂电池包 (3.7V+充电+升压5V/2.4A) | ¥19.00 | 内置电池+充电+升压+开关 |
| OLED | SSD1306 128×64 I2C | ~¥8 | 与 GY-87 共 I2C 总线 |
...
- [x] GY-87 + ESP32 杜邦线联调（I2C `0x68`/`0x77`）
- [ ] 载板 PCB v0.5：J2 按 GY-87 改走线并打样
- [ ] 面包板搭建原型，验证接线
- [ ] 实测椅子坐下/站起时的角度变化数据
- [ ] 确定安装位置和固定方式
